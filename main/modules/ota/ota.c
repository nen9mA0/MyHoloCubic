...
#ifdef CRC32_CHECK
        ESP_LOGI(TAG, "Image CRC: %08X", crc);
        client = ClientInit(url, "/upg_dev_crc");
        int crc_check_flag = false;
        if (client)
        {
            err = ClientOpen(client);
            data_read = ClientRead(client, ota_write_data, BUFFSIZE);
            if(data_read > 0)
            {
                uint32_t crc_get = Hex2Num(ota_write_data, data_read);
                ESP_LOGI(TAG, "Original Image CRC: %08X", crc_get);
                if(crc == crc_get)
                {
                    ESP_LOGI(TAG, "CRC Check Done");
                    crc_check_flag = true;
                }
            }
        }
